...
void test_IT01_TurnLeft_Timer_Logic(void) {
    ...
    Controller(trigger, false); // Tick 1 (Start)
...
void test_IT06_Backward_Loop_Logic(void) {
    ...
    // 1회차 후진 (5 Tick)
    for (int i = 0; i < 5; i++) Controller(blocked, false);
...
// IT-08: 모터(회전)와 청소기(PowerUp) 동시 제어 검증
...
void test_IT10_PowerUp_Extension(void) {
    ...
    Controller(o, true); // Tick 1 (Timer=1)
    Controller(o, false); // Tick 2 (Timer=2)
    Controller(o, true);  // Tick 3 (먼지 재발견 -> Timer=1 리셋되어야 함)
